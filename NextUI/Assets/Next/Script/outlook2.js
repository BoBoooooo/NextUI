...
function addNav(data) {
    ...
    $('.easyui-accordion li a').click(function () {
        ...
        if (!exist && url != undefined) {
            addTabWithID(title, url, icon,menuid);
        }
        else {
...
function addTabWithID(subtitle, url, icon,menuid) {
    var tabs = $('#tabs').tabs('tabs');
    var options = {
        title: subtitle,
        content: createFrameWithID(url, menuid),
        closable: true,
...
function addExtraTabWithID(subtitle, url, icon, menuid) {
    var tabs = $('#tabs').tabs('tabs');
    var options = {
        title: subtitle,
        content: createFrameWithID(url, menuid),
...
function createFrameWithID(url, menuid) {
    
    if (url.indexOf('?') == -1) {
        var s = '<iframe id="tab_' + menuid.replaceAll('-', '') + '_iframe" name="tab_' + menuid.replaceAll('-', '') + '_iframe"  scrolling="auto" frameborder="0"  src="' + url + '?appid=' + menuid + '&tabid=tab_' + menuid.replaceAll('-', '') + '" style="width:100%;height:99%;overflow:hidden"></iframe>';
    } else {
        var s = '<iframe id="tab_' + menuid.replaceAll('-', '') + '_iframe" name="tab_' + menuid.replaceAll('-', '') + '_iframe"  scrolling="auto" frameborder="0"  src="' + url + '&appid=' + menuid + '&tabid=tab_' + menuid.replaceAll('-', '') + '" style="width:100%;height:99%;overflow:hidden"></iframe>';
    }
    return s;
