...
{
	public class RoleAppBLL : BaseBLL<RoleApp>
	{
		private IRoleAppDAL roleAppDAL;
		public RoleAppBLL(): base()
		{
			...
			this.roleAppDAL = (IRoleAppDAL)base.baseDal;
		}
        /// <summary>
        /// 查询一个角色所有记录
        /// </summary>
        ...
        {
            return roleAppDAL.GetAllByRoleID(roleID);
        }
        ...
        {
            return roleAppDAL.GetAllDataTableByRoleID(roleID);
        }
        ...
        {
            return roleAppDAL.GetAllDataTable();
        }
        ...
        {
            return roleAppDAL.GetChildsDataTable(id);
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public List<RoleApp> GetAllChild(string id)
        {
            List<RoleApp> list = new List<RoleApp>();
        ...
        private void addChilds(List<RoleApp> list, string id)
        {
            var childs = GetChild(id);
            foreach (var child in childs)
            {
        ...
        {
            return roleAppDAL.UpdateSort(id, sort);
        }
        ...
        {
            return roleAppDAL.DeleteByRoleID(roleID);
        }
        /// <summary>
        /// 删除一个应用记录
        /// </summary>
        ...
        {
            return roleAppDAL.DeleteByAppID(appID);
        }
        ...
        {
            return roleAppDAL.GetMaxSort(id);
        }
        ...
        /// <returns></returns>
        public bool CopyRoleApp(string fromID, string toID)
        {
            var toRole = new RoleBLL().FindByID(toID);
            if (toRole == null)
            {
                return false;
            }
            using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
            {
                ...
                var root = apps.Find(p => p.PID == string.Empty);
                if (root != null)
                {
                    string fID = root.ID;
        ...
        private void insertChilds(string oldID, string newID, string roleID, List<RoleApp> apps)
        {
            var childs = apps.FindAll(p => p.PID == oldID);
            foreach (var child in childs)
            {
                string fID = child.ID;
        ...
        /// <param name="roleID"></param>
        /// <returns></returns>
        public RoleApp GetRootByRoleID(string roleID)
        {
            var roles = GetAllByRoleID(roleID);
            return roles.Find(p => p.PID == string.Empty);
        }
    }
}
