...
namespace Next.WorkFlow.BLL
{
	public class WorkFlowFormBLL : BaseBLL<WorkFlowForm>
	{
        ...
        public string GetValidatePropTypeRadios(string name, string value, string att = "")
        {
            ...
            return Next.WorkFlow.Utility.Tools.GetRadioString(items, name, att);
        }
        ...
        public string GetEventOptions(string name, string value, string att = "")
        {
            ...
            return Next.WorkFlow.Utility.Tools.GetOptionsString(items);
        }
        ...
        public string GetDefaultValueSelect(string value)
        {
            ...
            options.AppendFormat("<option value=\"@(Next.WorkFlow.Utility.DateTimeNew.ShortTime)\" {0}>短时间格式(23:59)</option>", "22" == value ? "selected=\"selected\"" : "");
            options.AppendFormat("<option value=\"@(Next.WorkFlow.Utility.DateTimeNew.LongTime)\" {0}>长时间格式(23时59分)</option>", "23" == value ? "selected=\"selected\"" : "");
            options.AppendFormat("<option value=\"@(Next.WorkFlow.Utility.DateTimeNew.ShortDateTime)\" {0}>短日期时间格式(2014-4-15 22:31)</option>", "24" == value ? "selected=\"selected\"" : "");
            options.AppendFormat("<option value=\"@(Next.WorkFlow.Utility.DateTimeNew.LongDateTime)\" {0}>长日期时间格式(2014年4月15日 22时31分)</option>", "25" == value ? "selected=\"selected\"" : "");
        ...
        public string GetOrgSelectTypeCheckboxs(string name, string value, string att = "")
        {
            ...
            return Next.WorkFlow.Utility.Tools.GetCheckBoxString(items, name, value.Split(','), att);
        }
        ...
        public string GetOptionsFromSql(string connID, string sql, string value)
        {
            ...
            return Next.WorkFlow.Utility.Tools.GetOptionsString(items.ToArray());
        }
        ...
        public string GetCheckboxFromSql(string connID, string sql, string name, string value, string attr = "")
        {
            ...
            return Next.WorkFlow.Utility.Tools.GetCheckBoxString(items.ToArray(), name, (value ?? "").Split(','), attr);
        }
        ...
        /// 查询一个分类所有记录
        /// </summary>
        public List<WorkFlowForm> GetAllByType(string id)
        {
            return workFlowFormDAL.GetAllByType(GetAllChildsIDString(id));
        }
